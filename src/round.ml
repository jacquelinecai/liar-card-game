...
let cList= [Number 1;Number 2;Number 3;Number 4;Number 5; Number 6; Number 7; Number 8;Number 9;Number 10; Jack; Queen; King]
let curr_round = ref 0
let card_round () = number_match (List.nth cList !curr_round)
let increment () = if !curr_round=11 then curr_round:=0 else curr_round:=!curr_round+1
...
let change_to_pass plyr = match plyr with 
|"Player 1"->(p.p1<-Pass)
|"Player 2"->(p.p2<-Pass)
|"Player 3"->(p.p3<-Pass)
|"Player 4"->(p.p4<-Pass)
  let order () = let y= (Random.int 4) in match y with
  |0->"Player 1"
  |1->"Player 2"
  |2->"Player 3"
  |_->"Player 4"
